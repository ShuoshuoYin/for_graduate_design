...
public class ThreeWordsTable {
	
	private static Analyzer analyzer = null; 
	private static Set<String> oneWordSet = null;
	
	private static boolean haveIntersection(String s0, String s1, String s2) {
	...
	private static void adjustOrder(String s1, String s2, String s3, int[] index) {
	...
	public static class Map extends Mapper<Object, Text, Text, Text> {
		...
		public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			...
			int[] index = new int[3];
			if(i == 3) {
				if(haveIntersection(words[0], words[1], words[2]))
					return ;
				adjustOrder(words[0], words[1], words[2], index);
				context.write(
						new Text(words[index[0]]+","
								+words[index[1]]+","
								+words[index[2]]), new Text(""));
			}
			else if(i == 4) {
				...
				for(int j=0; j<4; j++) {
					...
					if(!haveIntersection(ws[0], ws[1], ws[2])) {
						adjustOrder(ws[0], ws[1], ws[2], index);
						context.write(
							new Text(ws[index[0]]+","
									+ws[index[1]]+","
									+ws[index[2]]), new Text(""));
	...
	public static void main(String[] args) throws IllegalArgumentException, IOException, ClassNotFoundException, InterruptedException {
		...
		Job job = Job.getInstance(conf, "ThreeWordsTable");
		job.setJarByClass(ThreeWordsTable.class);
		...
		FileOutputFormat.setOutputPath(job, new Path("/apriori/three_words_table/threeTF_888"));
